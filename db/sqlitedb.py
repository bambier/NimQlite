...
class SQLiteDB(BaseDB):
    ...
    def __init__(self, database_name:str=f"{uuid4()}.sqlite3")-> None:
        ...
        self.logger.info(f"Creating databse")
    ...
    def create(self, table_name: str, values:list) ->bool:
        """
        ...
        try:
            ...
            self.logger.info(f"Inserting data in to database")
            self.execute("""
    ...
    def select_data_from_and(self, table_name, where:str, column:list=["*"]):
        ...
        try:
            ...
            self.logger.info(f"Selecting data from and")
            data = list(
                self.execute("""SELECT {0} FROM {1} WHERE {2};
    ...
    def select_data_from_or(self, table_name, where:str, column=["*"]):
        ...
        try:
            ...
            for key , value in where.items():
                data_sets.append(f"{key} = '{value}'")
            self.logger.info(f"Selecting from or")
            data = list(
                self.execute("""SELECT {0} FROM {1} WHERE {2};
    ...
    def execute(self, table_name, command:str):
        try:
            self.logger.info(f"Executing data")
    ...
    def commit(self)->bool:
        ...
        """
        try:
            self.logger.info(f"Commiting data")
            self.db.commit()
            return True
        except Exception as e:
            self.logger.error(f"[ERROR] {e}")
    ...
    def close(self) -> bool:
        ...
        """
        try:
            self.logger.info(f"CLosing database")
            self.db.close()
            return True
        except Exception as e:
            self.logger.error(f"[ERROR] {e}")
